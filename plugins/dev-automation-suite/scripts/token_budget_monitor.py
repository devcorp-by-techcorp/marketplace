...
Budgets are estimates until a phase has run once on the models actually
in use: tokenizers differ between model generations, so a figure carried
over from an earlier deployment is a guess, not a measurement.
...
other context, so an agent-teams run can be costed apart from the phase
it ran inside.
...
#: USD per million tokens, {input, output}. Published list prices, which are
#: not necessarily what a given account is billed — treat cost output as an
#: estimate and re-check these figures when a model generation changes.
#: Aliases resolve to the current model of that tier.
MODEL_PRICING = {
...
#: Fallback when a recorded model carries no published price. Assuming the
#: Opus rate over-reports rather than under-reports, which is the safe
#: direction for a budget monitor.
...
class PhaseUsage:
    ...
    #: Optional free-form attribution, e.g. `team:review-team`. Lets an
    #: agent-teams run account for its own cost separately from the phase
    #: total, which is the only way to tell an expensive team from an
    #: expensive phase.
...
def cmd_init(args: argparse.Namespace) -> None:
    ...
    print(json.dumps({
        ...
        'estimated_cost_usd': {
            'all_opus_5': round(calculate_cost(total_budget, 'claude-opus-5'), 2),
            'all_sonnet_5': round(calculate_cost(total_budget, 'claude-sonnet-5'), 2),
        },
    }, indent=2))
...
def cmd_record(args: argparse.Namespace) -> None:
    ...
    # Built as a variable rather than printed inline: the tag fields below are
    # conditional, and a caller that passed no --tag must see no tag keys at
    # all rather than nulls.
    result = {
        'status': 'RECORDED',
        'phase': args.phase,
...
def cmd_summary(args: argparse.Namespace) -> None:
    ...
    tag_summary = _aggregate_tags(state, getattr(args, 'tag', None))
    if tag_summary:
...
def main():
    ...
    record_parser.add_argument('--tag', help='Attribution tag, e.g. team:review-team')
    ...
    summary_parser.add_argument('--tag', help='Restrict tag breakdown to one tag')
    ...
    report_parser.add_argument('--tag', help='Restrict tag breakdown to one tag')
